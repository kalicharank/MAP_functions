...
msr_sanity_dailyCR_graph <- function(data_table, x_axis_date_breaks)
{
  ...
  # for debugging: data_table = map_daily_cr_data; x_axis_date_breaks = '1 day'
  ...
  ##scaling and adjusting date format
  g1 <- g1 + scale_x_date(breaks = date_breaks(x_axis_date_breaks), labels = date_format("%m/%d")) + scale_y_continuous(labels = percent)
